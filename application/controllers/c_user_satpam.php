...
class c_user_satpam extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model(['m_mutasi_jaga', 'm_peralatan_jaga', 'm_satpam']);
        // $this->load->library(['form_validation', 'template', 'upload', 'image_lib']);
    ...
    {
        $data_mutasi = $this->m_mutasi_jaga->get_all();
        $data = array(
    ...
    public function read($id_mutasi_jaga) 
    {
        $row = $this->m_mutasi_jaga->get_by_id($id_mutasi_jaga);
        $mutasi_peralatan = $this->m_mutasi_jaga->get_mutasi_peralatan($id_mutasi_jaga);
        ...
        if ($row) {
            $data = array(
		'id_mutasi_jaga' => $row->id_mutasi_jaga,
		'tanggal_jaga' => $row->tanggal_jaga,
		'shift' => $row->shift,
		'petugas_serah' => $row->petugas_serah,
		'petugas_terima' => $row->petugas_terima,
		'catatan_mutasi' => $row->catatan_mutasi,
		'dibuat_pada' => $row->dibuat_pada,
        'nama_unit' => $row->nama_unit,
        'mutasi_peralatan' => $mutasi_peralatan,
	    );
            $this->template->display('mutasi_jaga/v_mutasi_detail', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('c_mutasi_jaga'));
        }
    }

    

    public function input_mutasi_jaga() 
    {
        print_r($this->session->userdata());
                exit;

        $satpam=$this->m_satpam->get_all_by_id_unit("1");
    	$peralatan_jaga=$this->m_peralatan_jaga->get_all();
        $data = array(
        'id_mutasi_jaga' => set_value('id_mutasi_jaga'),
	    'id_unit' => set_value('id_umkm'),
	    'tanggal_jaga' => set_value('nama_perusahaan'),
	    'shift' => set_value('nama_pimpinan'),
	    'petugas_serah' => set_value('foto_pimpinan'),
	    'petugas_terima' => set_value('no_ktp'),
	    'catatan_mutasi' => set_value('alamat'),
	    'peralatan_jaga'=>$peralatan_jaga,
        'satpam'=>$satpam,
	);
        $this->template->user('satpam/v_mutasi_form', $data);
    }
    
    
    public function input_mutasi_action() 
    {
    // $this->_rules();
    $id_satpam   = $this->session->userdata('id_satpam');
    $nama_satpam = $this->session->userdata('nama_satpam');
    $id_unit   = $this->session->userdata('id_unit');

    echo "ID Satpam: " . $id_satpam . "<br>";
    echo "Nama Satpam: " . $nama_satpam . "<br>";
    echo "ID Unit: " . $id_unit;
    
    $approver_1 = $this->m_mutasi_jaga->get_approver_1_by_id_unit($id_unit);
    $approver_2 = $this->m_mutasi_jaga->get_approver_2_by_id_unit($id_unit);

    if($approver_1) { 
        $id_approver_1=$approver_1->id_user;
        $nama_approver_1=$approver_1->nama_user;
    } else { 
        $approver_1="";
    }

    if($approver_2) { 
        $id_approver_2=$approver_2->id_user;
        $nama_approver_2=$approver_2->nama_user;
    } else { 
        $approver_2="";
    }

    // ============================
        // 1️⃣ Simpan data utama mutasi jaga
        // ============================
        $data_mutasi = array(
            'id_unit'        => $id_unit,
            'tanggal_jaga'   => $this->input->post('tanggal_jaga', TRUE),
            'shift'          => $this->input->post('shift', TRUE),
            'petugas_serah'  => $this->input->post('petugas_serah', TRUE),
            'petugas_terima' => $this->input->post('petugas_terima', TRUE),
            'catatan_mutasi' => $this->input->post('catatan_mutasi', TRUE),
            'id_approver_1' => $id_approver_1,
            'nama_approver_1' => $nama_approver_1,
            'id_approver_2' => $id_approver_2,
            'nama_approver_2' => $nama_approver_2,
            'status' => "menunggu approval",
        );

        
        // Insert ke tabel mutasi jaga
        $this->m_mutasi_jaga->insert_mutasi($data_mutasi);

        // Ambil ID yang baru saja disimpan
        $id_mutasi_jaga = $this->db->insert_id();

        // ============================
        // Simpan detail peralatan jaga
        // ============================
        ...
        if (!empty($peralatan)) {
            ...
            $this->m_mutasi_jaga->insert_mutasi_peralatan_batch($data_detail);
        }

        //Harusnya ini di atas untuk mengecek apakah sudah disetting data approver atau belum, jika belum maka akan muncul notifikasi belum disetting
        // $approver_1 = $this->m_mutasi_jaga->get_approver_1_by_id_unit("1");
        // $approver_2 = $this->m_mutasi_jaga->get_approver_2_by_id_unit("1");

        // if($approver_1) { 
        //     $approver_1=$approver_1->id_user;
        //     $nama_approver_1=$approver_1->nama_user;
        // } else { 
        //     $approver_1="";
        // }
        ...
        // $data_approval = array(
        //     'id_mutasi_jaga'    => $id_mutasi_jaga,
        //     'id_petugas_terima' => $this->input->post('petugas_terima', TRUE),
        //     'id_approver_1' => $approver_1,
        //     'nama_approver_1' => $nama_approver_1,
        //     'id_approver_2' => $approver_2,
        //     'nama_approver_2' => $nama_approver_2,
        // );
        // Insert ke tabel approval mutasi
        //$this->m_mutasi_jaga->insert_approval_mutasi($data_approval);

        // ============================
        // 3️⃣ Redirect dan notifikasi
        // ============================
        $this->session->set_flashdata('message', 'Data mutasi jaga dan peralatan berhasil disimpan!');
        redirect(site_url('c_mutasi_jaga'));
    }
